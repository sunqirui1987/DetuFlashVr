...
final public class Panorama
{
	...
	public var effMatrix3D:Matrix3D = new Matrix3D();
	
	public var fisheyeLens:BitmapData
	...
	final public function render( viewData:ViewData, targetGraphics:Graphics = null,canvasinternal:Sprite = null):Boolean
	{ 
...
//			transform.prepend(effMatrix3D);
//			
//			trace(effMatrix3D.position,effMatrix3D.rawData);
//			
//			transform.appendTranslation(Math.random()*200,Math.random()*100,Math.random()*50)
//			transform.appendRotation(Math.random()*200,Vector3D.X_AXIS);
//			
//			transform.
		}
/* field of view or view bounds have changed and need to update frustum, as well as pixels per degree of the view area. */
		...
		//	x							y							z							w
			...
			if(isNaN(adjacent) || adjacent <= 0)
			{
				perspectiveProjection.focalLength = 0.000001
			}
			else
			{
...
//Profiler.instance.end("clip");
		...
		if ( targetGraphics && _graphicsData.length > 0) { 
			...
			/*
			if(canvasinternal != null && canvasinternal.height > 0)
			{
				
				var radius:Number  = canvasinternal.height/2;
				if(fisheyeLens == null || fisheyeLens.height != radius*2)
				{
					trace("canvasinternal enter" + canvasinternal.height);
				...
				displaceFilter = new DisplacementMapFilter(fisheyeLens,new Point(radius, 0), BitmapDataChannel.RED,BitmapDataChannel.BLUE,radius, 0);
				
				// Apply the filter
				canvasinternal.filters = [displaceFilter];
