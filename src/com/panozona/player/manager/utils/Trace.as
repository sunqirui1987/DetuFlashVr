...
package com.panozona.player.manager.utils {
	...
	public class Trace extends Sprite {
		...
		private function stageReady(e:Event = null):void {
			...
			qjPlayer = (this.parent as QjPlayer);
			
			traceData = qjPlayer.managerData.traceData;
			debugMode = qjPlayer.managerData.debugMode;
			...
			qjPlayer.manager.addEventListener(ViewEvent.BOUNDS_CHANGED, handleResize ,false, 0, true);
			handleResize();
		}
		...
		private function handleResize(e:Event = null):void {
			if (traceData.align.horizontal == Align.RIGHT) {
				btnOpen.x = qjPlayer.manager.boundsWidth - btnOpen.width;
				window.x = qjPlayer.manager.boundsWidth - traceData.size.width;
			}else if (traceData.align.horizontal == Align.LEFT) {
				btnOpen.x = 0;
				window.x = 0;
			}else{ // center
				btnOpen.x = (qjPlayer.manager.boundsWidth - btnOpen.width)*0.5;
				window.x = (qjPlayer.manager.boundsWidth - traceData.size.width) * 0.5;
			}
			if (traceData.align.vertical == Align.TOP) {
				btnOpen.y = 0;
				window.y = 0;
			}else if (traceData.align.vertical == Align.BOTTOM) {
				btnOpen.y = qjPlayer.manager.boundsHeight- btnOpen.height;
				window.y = qjPlayer.manager.boundsHeight - traceData.size.height;
			}else { // middle
				btnOpen.y = (qjPlayer.manager.boundsHeight - btnOpen.height)*0.5;
				window.y = (qjPlayer.manager.boundsHeight - traceData.size.height)*0.5;
			}
