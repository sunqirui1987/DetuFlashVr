...
package com.panozona.modules.compass.controller {
	...
	public class CompassController {
		...
		public function CompassController(compassView:CompassView, module:Module) {
			...
			_module.qjPlayer.manager.addEventListener(panoramaEventClass.PANORAMA_STARTED_LOADING, onPanoramaStartedLoading2, false, 0, true);
			...
			if (_compassView.compassData.settings.path != null){
				...
				compassLoader.load(new URLRequest(compassView.compassData.settings.path) ,new flash.system.LoaderContext(true) );
			}
		}
		...
		private function onPanoramaStartedLoading(loadPanoramaEvent:Object):void {
			...
			if (_compassView.compassData.windowData.open) {
				_module.qjPlayer.manager.runAction(_compassView.compassData.windowData.window.onOpen);
			}else {
				_module.qjPlayer.manager.runAction(_compassView.compassData.windowData.window.onClose);
			}
		}
		
		private function onPanoramaStartedLoading2(e:Event):void {
			currentDirection = _module.qjPlayer.manager.currentPanoramaData.direction;
		}
		...
		private function enterFrameHandler(event:Event):void{
			_compassView.axis.rotationZ = _module.qjPlayer.manager._pan + currentDirection;
		}
	}
}