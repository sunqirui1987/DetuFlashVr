...
package com.panozona.modules.mousecursor {
	...
	public class MouseCursor extends Module{
		...
		//[Embed(source="assert/cursors_21x21.png")]
		//private static var Bitmap_cursor:Class;
		
		
		
		public function MouseCursor(){
			super("MouseCursor", "1.2", "http://ouwei.cn/wiki/Module:MouseCursor");
		...
		override protected function moduleReady(moduleData:ModuleData):void {
			...
			mouseCursorData = new MouseCursorData(moduleData, qjPlayer);
			
	
			
			if(mouseCursorData.settings.path.indexOf("qj_templatexml") > -1 )
			{
				cursor = new Bitmap();
				...
				qjPlayer.manager.canvasInternal.addEventListener(MouseEvent.ROLL_OVER, onMouseOver, false, 0, true);
				qjPlayer.manager.canvasInternal.addEventListener(MouseEvent.ROLL_OUT, onMouseOut, false, 0, true);
				
				qjPlayer.manager.managedChildren.addEventListener(MouseEvent.ROLL_OVER, onMouseOverHotspots, false, 0, true);
				qjPlayer.manager.managedChildren.addEventListener(MouseEvent.ROLL_OUT, onMouseOutHotspots, false, 0, true);
				
				qjPlayer.manager.canvasInternal.addEventListener(MouseEvent.MOUSE_DOWN, onMousePress, false, 0, true);
				qjPlayer.manager.canvasInternal.addEventListener(MouseEvent.MOUSE_UP, onMouseRelease, false, 0, true);
				...
				qjPlayer.managerData.controlData.arcBallCameraData.addEventListener(cameraEventClass.ENABLED_CHANGE, onDragEnabledChange, false, 0, true);
				...
				cursorsLoader.load(new URLRequest(mouseCursorData.settings.path),new LoaderContext(true));
		...
		{/*
			var bitmapData:BitmapData = new Bitmap_cursor().bitmapData;
		...
		private function onDragEnabledChange(e:Object = null):void {
			dragMode = qjPlayer.managerData.controlData.arcBallCameraData.enabled;
			if (mousePress) {
				if (mousePressMove) {
					if (dragMode) {
						drawHandDrag();
