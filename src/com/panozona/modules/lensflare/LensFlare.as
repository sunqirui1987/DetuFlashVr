...
package com.panozona.modules.lensflare{
	...
	public class LensFlare extends Module {
		...
		public function LensFlare():void {
			...
			super("LensFlare", "1.0", "http://ouwei.cn/wiki/Module:LensFlare");
		}
		
		override protected function moduleReady(moduleData:ModuleData):void {
			lensFlareData = new LensFlareData(moduleData, qjPlayer);
		...
		private function onPanoramaLoaded(e:Event):void {
			var currentPano:String = qjPlayer.manager.currentPanoramaData.id;
			if (_panos[currentPano]) {
				...
				if (_panos[currentPano].path) {
					if (!_flaresLocalLoaded[currentPano]) {
						...
						loader.load(new URLRequest(_panos[currentPano].path),new LoaderContext(true));
					} else {
						setPositions();
					}
				} else {
					if (!_flaresLoaded) {
						...
						loader.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR, imageLost);
						loader.contentLoaderInfo.addEventListener(Event.COMPLETE, imageLoaded);
						loader.load(new URLRequest(lensFlareData.settings.path),new LoaderContext(true));
					} else {
						setPositions();
					}
				}
			...
			if (cellsCount == ((e.target as LoaderInfo).width + 1) / ((e.target as LoaderInfo).height + 1)) {
				...
				for (var i:int = 0; i < cellsCount; i++) {
					...
					if (_panos[currentPano].path) {
						...
						qjPlayer.manager.addChild(_flaresLocal[currentPano][i].image);
					} else {
						_flares[i] = flare;
						qjPlayer.manager.addChild(_flares[i].image);
					}
				}
		...
		private function lensFlareEffect(event:Event):void {
			if (_panos[qjPlayer.manager.currentPanoramaData.id]) {
				
				var panDist:Number = Math.abs(qjPlayer.manager._pan - fPan);
				if (panDist > 180) {
					panDist = 360 - panDist;
				}
				
				_fDistance = Math.sqrt(Math.pow(panDist, 2) + Math.pow(Math.abs(qjPlayer.manager._tilt - fTilt), 2));
				
				var level:Number = _panos[qjPlayer.manager.currentPanoramaData.id].brightness.level == -1 ?
					lensFlareData.settings.brightness.level : _panos[qjPlayer.manager.currentPanoramaData.id].brightness.level;	
					
				var distance:Number = _panos[qjPlayer.manager.currentPanoramaData.id].brightness.distance == -1 ?
					lensFlareData.settings.brightness.distance : _panos[qjPlayer.manager.currentPanoramaData.id].brightness.distance;
		...
		private function showFlares():void{
			...
			var w:int = qjPlayer.manager.boundsWidth;
			var h:int = qjPlayer.manager.boundsHeight;
			...
			if (fx <= w && fx > 0 && fy <= h && fy > 0) {
				...
				for each(var flare:Object in flares) {
					var flarePan:Number = validatePanTilt(fPan + validatePanTilt(qjPlayer.manager._pan - fPan) * flare.position);
					var flareTilt:Number = validatePanTilt(fTilt + validatePanTilt(qjPlayer.manager._tilt - fTilt) * flare.position);				
		...
		private function setBrightness(level:int):void {
			...
			qjPlayer.manager.canvas.transform.colorTransform = colorTransform;
			brightness = level;
		}
		...
		private function panToX(pan:Number):int {
			var w:Number = qjPlayer.manager.boundsWidth;
			var cPan:Number = qjPlayer.manager._pan;
			var fov:Number = qjPlayer.manager._fieldOfView;
		...
		private function tiltToY(tilt:Number):int {
			var w:Number = qjPlayer.manager.boundsWidth;
			var h:Number = qjPlayer.manager.boundsHeight;
			var cTilt:Number = qjPlayer.manager._tilt;
			var fov:Number = qjPlayer.manager._fieldOfView;
